...
try {
	...
	<div style='position: relative; width:100%; border-bottom: 1px solid #000;'>
		...
		<div style='width:300px; float:right; display: inline; text-align:right;'>
			<div>{DATE j/m/Y}</div>
			<div style='margin-top:8px;font-weight:bold;'>RELAÇÃO DE PROFISSIONAIS</div>
		</div> 
	 </div>
	";		
	...
	//Se for todas os Nomes
		...
		foreach ($row as $item){	
			...
			if ($item['ProfiTipo'] == 'F'){
				$sProfissional = $item['ProfiNome'];
				$iDocumento = formatarCPF_Cnpj($item['ProfiCpf']);
			} else {
				$sProfissional = $item['ProfiRazaoSocial'];
				$iDocumento = formatarCPF_Cnpj($item['ProfiCnpj']);
			}
		
			$html .= "
					<tr>
						<td style='padding-top: 8px; font-size: 11px;'>".$cont."</td>
						<td style='padding-top: 8px; font-size: 11px;'>".$sProfissional."</td>
						<td style='padding-top: 8px; font-size: 11px;'>".$iDocumento."</td>
						<td style='padding-top: 8px; font-size: 11px;'>".$item['ProfiContato']."</td>
						<td style='padding-top: 8px; font-size: 11px;'>".$item['ProfiCelular']."</td>
						<td style='padding-top: 8px; font-size: 11px;'>".$item['ProfiEmail']."</td>
					</tr>
					";
			$cont++;
		}
