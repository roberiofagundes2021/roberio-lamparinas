...
if(isset($_POST['inputFornecedorAnexoId'])){
	...
	$sql = "SELECT FnAneId, FnAneData, FnAneNome, FnAneArquivo, FnAneFornecedor
			FROM FornecedorAnexo
			WHERE FnAneId = $iFornecedorAnexo ";
...
if(isset($_POST['inputNome'])){
	...
	try{
		...
		$_UP['pasta'] = 'global_assets/anexos/fornecedor/';
		...
		if (move_uploaded_file($_FILES['inputArquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
			...
			$sql = "UPDATE FornecedorAnexo SET FnAneData = :iData, FnAneNome = :sNome, FnAneArquivo = :iArquivo, FnAneUsuarioAtualizador = :iUsuarioAtualizador
					WHERE FnAneId = :iFornecedorAnexo";
			...
			$result->execute(array(
							...
							':iArquivo' => $nome_final,
							':iiUsuarioAtualizador' => $_SESSION['UsuarId'],
							':iFornecedorAnexo' => $_POST['inputFornecedorAnexoId']
							));

		} else{

			$sql = "UPDATE FornecedorAnexo SET FnAneData = :iData, FnAneNome = :sNome, FnAneUsuarioAtualizador = :iUsuarioAtualizador
					WHERE FnAneId = :iFornecedorAnexo";
			$result = $conn->prepare($sql);
	...
	<script type="text/javascript" >
        ...
        $(document).ready(function() {
			...
			$('#enviar').on('click', function(e){
				...
				var arquivoSalvo = $('#inputFornecedorAnexoArquivo').val();	
		...
		<div class="content-wrapper">
			...
			<div class="content">		
				...
				<div class="card">
					...
					<form name="formFornecedorAnexo" id="formFornecedorAnexo" method="post" enctype="multipart/form-data" class="form-validate-jquery">
						<div class="card-header header-elements-inline">
							<h5 class="text-uppercase font-weight-bold">Editar Anexo "<?php echo $row['FnAneArquivo']; ?>"</h5>
						</div>
						
						<input type="hidden" id="inputFornecedorAnexoId" name="inputFornecedorAnexoId" value="<?php echo $row['FnAneId']; ?>">
						<input type="hidden" id="inputFornecedorAnexoNome" name="inputFornecedorAnexoNome" value="<?php echo $row['FnAneNome']; ?>">
						<input type="hidden" id="inputFornecedorAnexoArquivo" name="inputFornecedorAnexoArquivo" value="<?php echo $row['FnAneArquivo']; ?>">
							...
							<div class="row">
								...
								<div class="col-lg-9">
									<div class="form-group">
										...
										<input type="text" id="inputNome" name="inputNome" class="form-control" placeholder="Descrição" value="<?php echo $row['FnAneNome']; ?>" required autofocus>
									</div>
								</div>
                            </div>	
                            <div class="row">
								<div class="col-lg-12">
									...
									<input type="file" id="inputArquivo" name="inputArquivo" class="form-control" value="<?php echo $row['FnAneArquivo']; ?>">
								</div>						
							</div>
							<div class="row">	
								<div class="col-lg-12">
									<div class="form-group">
										<span style="color:red;"><?php echo $row['FnAneArquivo']; ?></span>									
							...
							<div class="row" style="margin-top: 30px;">
								<div class="col-lg-12">								
									<div class="form-group">
										...
										<a href="fornecedorAnexo.php" class="btn btn-basic">Cancelar</a>
									</div>
								</div>
							</div>
						</form>								
