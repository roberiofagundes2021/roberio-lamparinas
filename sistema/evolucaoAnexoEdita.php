...
if(isset($_POST['inputEvolucaoAnexoId'])){
	...
	$iEvolucaoAnexo = $_POST['inputEvolucaoAnexoId'];
		
	$sql = "SELECT EnEAnId, EnEAnData, EnEAnNome, EnEAnArquivo, EnEAnEnfermagemEvolucao
			FROM EnfermagemEvolucaoAnexo
			WHERE EnEAnId = $iEvolucaoAnexo ";
...
if(isset($_POST['inputNome'])){
	...
	try{
		...
		$_UP['pasta'] = 'global_assets/anexos/evolucaoEnfermagem/';
		...
		if (move_uploaded_file($_FILES['inputArquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
			...
			$sql = "UPDATE EnfermagemEvolucaoAnexo SET EnEAnData = :iData, EnEAnNome = :sNome, EnEAnArquivo = :iArquivo, EnEAnUsuarioAtualizador = :iUsuarioAtualizador
					WHERE EnEAnId = :iEnfermagemAnexo";
			...
			$sql = "UPDATE EnfermagemEvolucaoAnexo SET EnEAnData = :iData, EnEAnNome = :sNome, EnEAnUsuarioAtualizador = :iUsuarioAtualizador
					WHERE EnEAnId = :iEnfermagemAnexo";
			$result = $conn->prepare($sql);
	...
	<script type="text/javascript" >
        ...
        $(document).ready(function() {
			...
			$('#enviar').on('click', function(e){
				...
				var arquivoSalvo = $('#inputEvolucaoAnexoArquivo').val();	
		...
		<div class="content-wrapper">
			...
			<div class="content">		
				...
				<div class="card">
					...
					<form name="formEvolucaoAnexo" id="formEvolucaoAnexo" method="post" enctype="multipart/form-data" class="form-validate-jquery">
						<div class="card-header header-elements-inline">
							<h5 class="text-uppercase font-weight-bold">Editar Anexo "<?php echo $row['EnEAnArquivo']; ?>"</h5>
						</div>
						
						<input type="hidden" id="inputEvolucaoAnexoId" name="inputEvolucaoAnexoId" value="<?php echo $row['EnEAnId']; ?>">
						<input type="hidden" id="inputEvolucaoAnexoNome" name="inputEvolucaoAnexoNome" value="<?php echo $row['EnEAnNome']; ?>">
						<input type="hidden" id="inputEvolucaoAnexoArquivo" name="inputEvolucaoAnexoArquivo" value="<?php echo $row['EnEAnArquivo']; ?>">
							...
							<div class="row">
								...
								<div class="col-lg-9">
									<div class="form-group">
										...
										<input type="text" id="inputNome" name="inputNome" class="form-control" placeholder="Descrição" value="<?php echo $row['EnEAnNome']; ?>" required autofocus>
									</div>
								</div>
                            </div>	
                            <div class="row">
								<div class="col-lg-12">
									...
									<input type="file" id="inputArquivo" name="inputArquivo" class="form-control" value="<?php echo $row['EnEAnArquivo']; ?>">
								</div>						
							</div>
							<div class="row">	
								<div class="col-lg-12">
									<div class="form-group">
										<span style="color:red;"><?php echo $row['EnEAnArquivo']; ?></span>									
							...
							<div class="row" style="margin-top: 30px;">
								<div class="col-lg-12">								
									<div class="form-group">
										...
										<a href="evolucaoAnexo.php" class="btn btn-basic">Cancelar</a>
									</div>
								</div>
							</div>
						</form>								
