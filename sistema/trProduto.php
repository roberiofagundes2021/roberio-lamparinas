											...
											<div class="form-group">
												...
												<select id="cmbProduto" name="cmbProduto" class="form-control multiselect-filtering" multiple="multiple" data-fouc>
													...
													if ($rowParametro['ParamProdutoOrcamento'] == 0 || count($aProdutos1) >= 1) {
														...
														$rowProduto = $result->fetchAll(PDO::FETCH_ASSOC);
														foreach ($rowProduto as $item) {
															if (in_array($item['ProduId'], $aProdutos) or $countProdutoUtilizado2 == 0) {
																$seleciona = "selected";
							...
							<div class="card">
								...
								<div class="card-body">
									...
									if ($rowParametro['ParamProdutoOrcamento'] == 0 || count($aProdutos1) >= 1) {
										...
										$sql = "SELECT ProduId, ProduNome, ProduDetalhamento, ProduUnidadeMedida, TRXPrQuantidade
									            FROM Produto
										...
										if (!$count) {
											$sql = "SELECT ProduId, ProduNome, ProduDetalhamento, ProduUnidadeMedida
										            FROM Produto
										            LEFT JOIN UnidadeMedida on UnMedId = ProduUnidadeMedida
